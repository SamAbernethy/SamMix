...
void ppi0 :: RebinData() // very long variable names, but this can be changed later
{
    ...
    for ( Int_t k = 1; k < newdatapoints; k++ ) {
        ...
        propagatederror[k] = (2./(pow(sumofyield0[k] + sumofyield1[k], 1.)))*sqrt(pow(sumofyield0[k], 2.)*pow(sumofyield1errorsquares[k], 1.) + pow(sumofyield1[k], 2.)*pow(sumofyield0errorsquares[k], 1.)) ;
